...
export function ColorView({
  ...
  return (
    <div className="flex flex-1 min-h-0">
      ...
      {/* Right — Color Picker */}
      <div className="flex-1 border-l border-white/[0.04] p-4 flex flex-col justify-center">
        {selectedKey !== null && selectedConfig ? (
          selectedConfig.override_enabled ? (
            ...
            <div className="text-center space-y-3">
              ...
              <button
                type="button"
                className="text-[11px] text-foreground/60 hover:text-foreground border border-white/10 rounded px-3 py-1.5 transition-colors"
                onClick={() => onToggleOverride(selectedKey)}
              >
                Enable override
              </button>
